...
def can_receive_voice(provider: ChannelProvider) -> bool:
    """Check if a provider can receive voice/audio messages.

    Used by the voice pipeline to route inbound audio messages
    to the correct provider for download and transcription.
    """
    return ChannelCapability.RECEIVE_VOICE in provider.capabilities


def can_send_voice(provider: ChannelProvider) -> bool:
    """Check if a provider can send voice/audio messages.

    NOTE: Per user constraint, the motor responds in TEXT only.
    This check exists for completeness and future use.
    """
    return ChannelCapability.SEND_VOICE in provider.capabilities
