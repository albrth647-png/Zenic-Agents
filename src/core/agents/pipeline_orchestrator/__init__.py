...
class PipelineOrchestrator(PipelineOrchestratorCoreMixin, PipelineOrchestratorExtraMixin):
    ...
    def __init__(self, mini_ai=None, smart_memory=None, semantic_engine=None) -> None:
        ...
        # ── Phase 7: Transport ──
        self._text_channel_agent = TextChannelAgent(**self._ik)
        self._voice_channel_agent = VoiceChannelAgent(**self._ik)
        # Channel infrastructure will be wired via wire_channels() after startup
    ...
    def wire_channels(self, registry: Any, router: Any = None) -> None:
        """Wire channel infrastructure to transport agents.

        Must be called after __init__ once the AdapterRegistry
        and ChannelRouter are available.

        Args:
            registry: AdapterRegistry instance.
            router: ChannelRouter instance (optional but recommended).
        """
        self._text_channel_agent.wire(registry=registry, router=router)
        # Also wire A52 with registry for audio download from providers
        if self._voice_pipeline is not None:
            self._voice_channel_agent.wire(
    ...
    def wire_voice_pipeline(
        ...
        Initializes the VoicePipeline with the given STT configuration
        and wires it to the A52 VoiceChannelAgent.

        If no config is provided, auto-detects the best available
        STT backend (faster_whisper → whisper → cloud → dummy).

        Must be called before A52 can transcribe audio.

        Args:
            stt_config: STT backend configuration. Auto-detects if None.

        Returns:
            The initialized VoicePipeline instance.
        """
        self._voice_pipeline = VoicePipeline(stt_config=stt_config)
        self._voice_channel_agent.wire(
            voice_pipeline=self._voice_pipeline,
