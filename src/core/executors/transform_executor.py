...
class TransformExecutor(ActionExecutor):
    ...
    def _filter(self, data, config):
        if not isinstance(data, list):
            raise ValueError(f"filter requires a list, got {type(data).__name__}")
    ...
    def _sort(self, data, config):
        if not isinstance(data, list):
            raise ValueError(f"sort requires a list, got {type(data).__name__}")
    ...
    def _aggregate(self, data, config):
        if not isinstance(data, list):
            raise ValueError(f"aggregate requires a list, got {type(data).__name__}")
        ...
        if group_by:
            return self._aggregate_grouped(data, aggregation, value_field, group_by)
    ...
    def _aggregate_grouped(self, data, aggregation, value_field, group_by):
        ...
        for gk, items in groups.items():
            ...
            if aggregation == "count":
                ...
                result[gk] = max(vals) if vals else None
    ...
    def _format_convert(self, data, config):
        ...
        if fmt == "json_to_csv":
            return self._json_to_csv(data, sep)
        elif fmt == "csv_to_json":
            return self._csv_to_json(data, sep)
        elif fmt == "flatten":
            return self._flatten(data)
        elif fmt == "nest":
            return self._nest(data, config.get("nest_key", ""))
    ...
    def _flatten(self, data, prefix=""):
        if not isinstance(data, dict):
            raise ValueError("flatten requires a dict")
        result = {}
        for k, v in data.items():
            fk = f"{prefix}.{k}" if prefix else k
            if isinstance(v, dict):
                result.update(self._flatten(v, fk))
            else:
                result[fk] = v
        return result
    ...
    def _nest(self, data, nest_key=""):
        if not isinstance(data, dict):
            raise ValueError("nest requires a dict")
        result = {}
        for k, v in data.items():
            parts = k.split(".")
            current = result
    ...
    def _merge(self, data, config):
        ...
        if not isinstance(data, list) or not isinstance(merge_data, list):
            raise ValueError("merge requires two lists")
        if not merge_on:
            raise ValueError("merge requires 'merge_on' field")
        ...
        for item in data:
            if isinstance(item, dict) and merge_on in item:
                ...
                for k, v in right_idx.get(str(item[merge_on]), {}).items():
                    if k != merge_on and k not in merged:
                        merged[k] = v
    ...
    def _deduplicate(self, data, config):
        if not isinstance(data, list):
            raise ValueError("deduplicate requires a list")
        keys = config.get("keys", [])
        key = config.get("key", "")
