...
class CertifiedBlueprint:
    ...
    def __init__(
        ...
        executor_schemas: dict[str, Any] | None = None,
        rules: list[BusinessRuleDef] | None = None,
        actions: list[ActionTemplateDef] | None = None,
        monitor_hooks: list[MonitorHook] | None = None,
    ) -> None:
    ...
    def add_entity(self, entity: DBEntitySchema) -> None:
        ...
        if entity.name in existing_names:
            ...
            self.db_schema.entities = [e for e in self.db_schema.entities if e.name != entity.name]
        self.db_schema.entities.append(entity)

    def get_entity(self, name: str) -> DBEntitySchema | None:
    ...
    def add_monitor_hook(self, hook: MonitorHook) -> None:
        ...
        if hook.monitor_id in existing_ids:
            ...
            self.monitor_hooks = [h for h in self.monitor_hooks if h.monitor_id != hook.monitor_id]
    ...
    def add_action(self, action: ActionTemplateDef) -> None:
        ...
        if action.template_id in existing:
            self.actions = [a for a in self.actions if a.template_id != action.template_id]
        self.actions.append(action)

    def get_action_by_name(self, name: str) -> ActionTemplateDef | None:
    ...
    def to_dict(self, include_signature: bool = True) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {
            ...
            "monitors": {h.monitor_id: _monitor_hook_to_dict(h) for h in self.monitor_hooks},
        }
        return result
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_fields": sum(len(e.fields) for e in self.db_schema.entities),
