...
impl MemoryChip {
    ...
    fn start_episode(
        ...
        let episode = lifecycle.start_episode(mapping).map_err(|e| {
            PyRuntimeError::new_err(format!("Failed to start episode: {}", e))
        })?;
        Ok(episode.id.clone())
    }
    ...
    fn stats(&self, py: Python<'_>) -> PyResult<Py<PyDict>> {
        ...
        let total_mappings = {
            ...
            graph.count_mappings("__unauthenticated__").unwrap_or(0)
