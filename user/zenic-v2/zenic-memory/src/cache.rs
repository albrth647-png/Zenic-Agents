...
use crate::types::{MemoryTierLimits, SemanticMapping, SubscriptionTier};
...
impl MemoryCache {
    ...
    /// [`MemoryTierLimits::for_tier`] which maps the tier to its LRU cache size.
    pub fn new_for_tier(tier: SubscriptionTier) -> Self {
        Self::new(MemoryTierLimits::for_tier(tier).lru_cache_size)
    }
