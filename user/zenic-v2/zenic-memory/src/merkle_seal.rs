...
impl MerkleSeal {
    ...
    /// Seals an approval action with BLAKE3(approval_data).
    ///
    /// This is used when approving a mapping (before the full Merkle seal
    /// via `seal_mapping()`), to avoid storing placeholder hashes like
    /// `"approved_{id}"`. The approval data includes the mapping_id,
    /// evidence review, justification length, and risk acknowledgment.
    pub fn seal_approval(&mut self, approval_data: &str) -> Result<String, MemoryError> {
        let hash = blake3::hash(approval_data.as_bytes());
        let hash_hex = hex::encode(hash.as_bytes());
