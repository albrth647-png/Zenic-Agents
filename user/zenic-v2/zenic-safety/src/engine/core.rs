//! Domain Safety Gate — Core pipeline.
//!
//! The `DomainSafetyGate` struct, its constructor, the full 4-layer `check()`
...
/// - High-risk compliance failures → at least APPROVE
pub struct DomainSafetyGate {
    pub(super) domain_rules: DomainRuleSet,
    pub(super) compliance_engine: ComplianceEngine,
}
...
impl DomainSafetyGate {
    ...
    pub fn check(
        ...
        // ── Layer 3: Compliance validation (HARD BLOCK) ───────
        let compliance_results = self
            .compliance_engine
            .check_category(niche_category, action_type, config);

        // Critical compliance violations → DENY (hard block)
        ...
        // High-risk compliance violations → at least APPROVE (hard block)
        if !has_critical {
            for cr in &compliance_results {
                if cr.risk_level == "high" && !cr.compliant {
                    if current_verdict < SafetyVerdict::Approve {
    ...
    pub(crate) fn classify_action(action_type: &str, config: &serde_json::Value) -> ActionCategory {
