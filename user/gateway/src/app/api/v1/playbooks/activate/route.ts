...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!selectedTier) {
      return NextResponse.json(
        { error: "selectedTier is required (starter, business, enterprise, on_premise_enterprise, or trial)", code: "VALIDATION_ERROR" },
        { status: 400 },
      );
    }
    ...
    if (!validTiers.includes(selectedTier)) {
      return NextResponse.json(
        { error: `Invalid selectedTier "${selectedTier}". Must be one of: starter, business, enterprise, on_premise_enterprise, trial`, code: "VALIDATION_ERROR" },
        { status: 400 },
      );
    }
