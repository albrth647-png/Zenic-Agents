...
export class PlaybookEngine {
  ...
  async createPlaybook(
    ...
    sourceYaml?: string,
  ): Promise<import("./hooks").PlaybookDbRecord> {
    try {
   ...
   */
  async getPlaybook(playbookId: string): Promise<import("./hooks").PlaybookDbRecord | null> {
    try {
   ...
   */
  async deactivatePlaybook(playbookId: string): Promise<import("./hooks").PlaybookDbRecord> {
    try {
      const existing = await db.playbook.findFirst({
        where: { playbookId },
      });
  ...
  async activatePlaybook(
    ...
    try {
      ...
      try {
        ...
        roiProjection = calculateRoiInline(roiConfig.baseline, roiConfig.projected, monthlyCostUsdt);
      }
