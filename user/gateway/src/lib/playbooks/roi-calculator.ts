...
function resolveMonthlyCost(
  tiers: Array<{ name: string; price_usdt: number; price_usd?: number }> | undefined,
  selectedTier: string,
): number {
  ...
  if (tier) {
    return tier.price_usdt ?? tier.price_usd ?? 0;
  }

  // Fallback: return the middle tier (Business) or first tier
  return tiers.length >= 2 ? (tiers[1].price_usdt ?? tiers[1].price_usd ?? 0) : (tiers[0].price_usdt ?? tiers[0].price_usd ?? 0);
}
