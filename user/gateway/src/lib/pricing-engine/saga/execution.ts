// ─── Zenic-Agents v3 — Saga Execution Engine ─────────────────────────────
// USDT TRC20 ONLY. Step action handlers and the core saga execution logic.
// Executes steps sequentially with compensating rollback on failure,
// pauses sagas awaiting external input, and resumes paused sagas.
...
export const stepHandlers: Record<string, StepHandler> = {
  ...
  async db_restore_usage_records(_input) {
  ...
  async await_admin_confirmation(_input) {
...
export async function executeSaga(
  ...
  if (currentStatus === "compensating") {
    ...
    for (let i = failedStepIndex - 1; i >= 0; i--) {
      ...
      const compHandler = compensationHandlers[step.compensating_action];
      if (compHandler) {
        try {
          await compHandler(input, stepOutputs[i] || {});
          await db.sagaStepRecord.update({
            where: { id: stepRecord.id },
        ...
        // Try step handler as compensating action
        ...
        if (compensatingAction) {
          ...
          // No compensation handler — best-effort
          await db.sagaStepRecord.update({
            where: { id: stepRecord.id },
            data: { status: "compensated", compensationCompletedAt: new Date() },
          });
...
export async function resumeSaga(
  ...
  if (!sagaExecution) {
    return {
      executionId,
      sagaType: "cancellation" as SagaTypeName,
      status: "failed",
      currentStepIndex: 0,
      totalSteps: 0,
      completedSteps: 0,
