...
from ._constants import PRIORITY_TO_IMPORTANCE, VALID_MODES
from ._rendering import (
    build_confirmation_html,
    build_confirmation_text,
    render_fields_table,
)
from ._send_methods import _SendMixin
...
class EmailChannelProvider(_SendMixin):
    ...
    def _message_to_config(self, message: ChannelMessage) -> dict[str, Any]:
        ...
        # Fields → HTML table
        if message.fields:
            fields_html = render_fields_table(message.fields)
            if config.get("html"):
