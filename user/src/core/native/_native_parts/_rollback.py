"""
Coordinated Rollback (A3) — Atomic cross-resource operations.

Provides: snapshot_file, restore_file, verify_rollback_readiness, file_hash
"""
...
def snapshot_file(source_path: str, backup_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "source_path": source_path, "backup_path": backup_path, "error": str(exc)}
    ...
    for res in resources:
        ...
        if rtype == "file":
            ...
            if bk is None or not bk.exists():
                failed.append({"resource_type": rtype, "backup_path": bp, "reason": "Backup file does not exist"})
                continue
            try:
                data = bk.read_bytes()
                ...
                failed.append({"resource_type": rtype, "backup_path": bp, "reason": str(exc)})
        else:
            verified += 1

    return {"all_verified": len(failed) == 0, "total_resources": total, "verified_count": verified, "failed": failed}


def file_hash(file_path: str) -> str:
    """Compute the BLAKE3/SHA-256 hash of a file."""
    if HAS_NATIVE:
        from ._loader import _rust_file_hash
