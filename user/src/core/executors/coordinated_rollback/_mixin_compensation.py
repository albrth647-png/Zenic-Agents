...
class CompensationMixin:
    ...
    def _compensate_file(
        ...
        if operation == "create":
            ...
            if not backup_path:
                raise ValueError(f"No backup_path for DELETE file rollback in action {action_id}")
            ...
            if not backup_path:
                raise ValueError(f"No backup_path for MODIFY file rollback in action {action_id}")
