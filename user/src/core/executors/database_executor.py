...
class DatabaseExecutor(ActionExecutor):
    ...
    async def execute(self, config: dict[str, Any], context: dict[str, Any]) -> ActionResult:
        ...
        if operation not in valid_ops:
            return ActionResult(
                False,
                {"operation": operation},
                f"Invalid DB operation: {operation}. Must be one of {valid_ops}",
        ...
        if not _validate_sql(query):
            return ActionResult(
                False, {"query": query}, "SQL validation failed: dangerous pattern detected", self._elapsed_ms(start)
            )
        try:
    ...
    async def _backup(self, db_path, destination, start):
        ...
        if not os.path.exists(db_path):
            return ActionResult(
                False, {"db_path": db_path}, f"Database file not found: {db_path}", self._elapsed_ms(start)
            )
        try:
            if not destination:
            ...
            return ActionResult(
                True,
                {"source": db_path, "destination": destination, "size_bytes": size},
    ...
    async def _execute_script(self, db_path, script, start):
        ...
        for stmt in statements:
            if not _validate_sql(stmt):
                logger.warning("DatabaseExecutor: Script mode blocked dangerous statement: %s", stmt[:100])
                return ActionResult(
                    False,
