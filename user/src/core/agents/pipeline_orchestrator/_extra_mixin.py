...
class PipelineOrchestratorExtraMixin:
    ...
    def _execute_automation_operation(
        ...
        action_result = self._run_agent(
            self._action_inferrer,
            {
                "description": message,
        ...
        schedule_result = self._run_agent(
            self._schedule_parser,
            {
                "description": message,
                "trigger_type": trigger_result.type if trigger_result else "manual",
            },
        ...
        condition_result = self._run_agent(
            self._condition_extractor,
            {
                "description": message,
            },
        )
        ...
        name_result = self._run_agent(
            self._automation_namer,
            {
                "description": message,
                "trigger_type": trigger_result.type if trigger_result else "manual",
        ...
        workflow_result = self._run_agent(
    ...
    def _execute_reasoning_operation(
        ...
        steps_result = self._run_agent(
            self._step_decomposer,
            {
                "query": message,
        ...
        confidence_result = self._run_agent(
        ...
        conclusion_result = self._run_agent(
            self._conclusion_extractor,
            {
                "reasoning_result": reasoning_result,
    ...
    def _is_automation_intent(message: str, intent_result: Any) -> bool:
        ...
        return bool(
            intent_result
            and isinstance(intent_result, IntentResult)
            and intent_result.operation == "CREATE"
            and "automat" in message.lower()
        )
    ...
    def _is_reasoning_intent(message: str, intent_result: Any) -> bool:
        ...
        if (
            intent_result
            and isinstance(intent_result, IntentResult)
            and intent_result.operation == "ANALYZE"
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mini_ai_loaded": (self._mini_ai is not None and getattr(self._mini_ai, "is_loaded", False)),
            "agents_wired": {
