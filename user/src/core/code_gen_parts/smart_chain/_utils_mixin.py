...
class SmartChainUtilsMixin:
    ...
    def _detect_task_type(description: str) -> str:
        ...
        if any(
            kw in desc for kw in ["crud", "create", "read", "update", "delete", "service", "api", "resource", "manage"]
        ):
            return "crud"
        if any(kw in desc for kw in ["auth", "jwt", "login", "token", "password", "registro", "signup"]):
            return "auth"
        if any(
            kw in desc
            for kw in ["stripe", "payment", "email", "smtp", "telegram", "webhook", "integration", "pago", "correo"]
        ):
            return "integration"
        if any(kw in desc for kw in ["analytics", "report", "dashboard", "stats", "metric", "analisis"]):
            return "analytics"
        return "generic"
