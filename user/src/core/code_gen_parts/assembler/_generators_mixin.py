...
class AssemblerGeneratorsMixin:
    ...
    def _build_crud_process(self, entity_name: str, table_name: str, fields: list[dict]) -> str:
    ...
    def _process(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        db_path = payload.get("db_path", "{table_name}.sqlite")
        ...
        if action == "create":
            ...
            try:
                ...
                conn.close()
            return {{"success": True, "action": "create", "entity": "{entity_name}", "id": last_id}}
            ...
            try:
                ...
                conn.close()
            return {{"success": True, "data": row, "entity": "{entity_name}"}}
            ...
            try:
                conn.execute(
                    "UPDATE {table_name} SET " + set_clause + " WHERE id = ?", values
                )
                conn.commit()
                affected = conn.execute("SELECT changes()").fetchone()[0]
            finally:
                conn.close()
            return {{"success": True, "action": "update", "entity": "{entity_name}", "affected": affected}}
            ...
            try:
                ...
                conn.close()
            return {{"success": True, "action": "delete", "entity": "{entity_name}", "affected": affected}}
            ...
            try:
                cursor = conn.execute(
                    "SELECT * FROM {table_name} LIMIT ? OFFSET ?", (limit, offset)
                )
                rows = [dict(r) for r in cursor.fetchall()]
            finally:
                conn.close()
            return {{"success": True, "data": rows, "entity": "{entity_name}", "count": len(rows)}}
            ...
            try:
                cursor = conn.execute(
                    "SELECT * FROM {table_name} WHERE " + column + " LIKE ?", ("%" + query + "%",)
                )
                rows = [dict(r) for r in cursor.fetchall()]
            finally:
                conn.close()
            ...
            conn = sqlite3.connect(db_path)
            try:
                cursor = conn.execute("SELECT COUNT(*) as total FROM {table_name}")
...
'''  # noqa: S608
    ...
    def _build_analytics_process(self, entity_name: str, table_name: str, fields: list[dict]) -> str:
        ...
        """
        numeric_fields = [
            f for f in fields if f.get("type", "").lower() in ("float", "int", "integer", "number", "decimal")
        ]
    ...
    def _process(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action == "summary":
            ...
            conn.row_factory = sqlite3.Row
            try:
                cursor = conn.execute("SELECT COUNT(*) as total, AVG({default_metric}) as avg_{default_metric} FROM {table_name}")
                row = dict(cursor.fetchone()) if cursor.fetchone() else {{}}
            finally:
                conn.close()
            return {{"success": True, "summary": row, "entity": "{entity_name}"}}
            ...
            try:
                cursor = conn.execute(
                    "SELECT date(created_at) as period, " + metric + " FROM {table_name} GROUP BY period ORDER BY period"
                )
                rows = [dict(r) for r in cursor.fetchall()]
            finally:
                conn.close()
            return {{"success": True, "data": rows, "metric": metric, "entity": "{entity_name}"}}
            ...
            try:
                ...
                conn.close()
            return {{"success": True, "distribution": rows, "entity": "{entity_name}"}}
            ...
            try:
                cursor = conn.execute(
                    ...
                    "FROM {table_name} WHERE created_at >= date('now', '-' || ? || ' days') "
...
'''  # noqa: S608
    ...
    def _build_notification_process(self, entity_name: str, fields: list[dict]) -> str:
        ...
        return f'''
    def _process(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Notification for {entity_name} — REAL sending via smtplib."""
        import smtplib
        ...
        if action == "send":
            ...
            if channel == "email" and recipient:
                msg = MIMEMultipart()
                msg["From"] = smtp_user or "noreply@{entity_name}.local"
                msg["To"] = recipient
                msg["Subject"] = subject
                ...
                try:
                    ...
                    server.quit()
                    return {{"success": True, "channel": "email", "recipient": recipient, "entity": "{entity_name}"}}
                except Exception as e:
                    return {{"success": False, "error": str(e), "entity": "{entity_name}"}}
            return {{"success": True, "channel": channel, "message": "logged", "entity": "{entity_name}"}}

        elif action == "broadcast":
            recipients = payload.get("recipients", [])
            message = payload.get("message", "")
            subject = payload.get("subject", "Broadcast from {entity_name}")
            smtp_host = payload.get("smtp_host", "localhost")
            smtp_port = payload.get("smtp_port", 587)
            smtp_user = payload.get("smtp_user", "")
            smtp_pass = payload.get("smtp_pass", "")
            ...
            import logging
            _logger = logging.getLogger("{entity_name}")
            level = payload.get("level", "info")
            message = payload.get("message", "")
            getattr(_logger, level, _logger.info)(message)
            return {{"success": True, "action": "log", "entity": "{entity_name}"}}
