...
///
/// FIX: Aligned with zenic-safety's `SafetyVerdict::severity()` to ensure
/// cross-implementation parity. Both implementations now agree:
///   ALLOW=0, CONFIRM=1, APPROVE=2, DENY=3, RATE_LIMITED=3
///
/// Previously pybridge had Deny=4 > RateLimited=3 while zenic-safety had
/// Deny=3 == RateLimited=3. This mismatch could cause different worst-match
/// selection behavior between the two implementations.
