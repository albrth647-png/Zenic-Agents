...
///
/// FIX: Uses `Instant`-based monotonic time instead of `SystemTime::now()`.
/// `SystemTime` is subject to NTP adjustments and clock jumps, which can
/// bypass the rate limiter by shifting time backwards. Monotonic time
/// only moves forward, making the rate limiter resistant to clock manipulation.
///
/// The implementation uses `Instant::now()` relative to a fixed epoch
/// established at program start, providing both monotonic guarantees
/// and Unix-epoch-compatible timestamps.

use std::time::Instant;

/// Fixed reference point for monotonic time calculations.
/// Established once at program startup.
static EPOCH: once_cell::sync::Lazy<(std::time::SystemTime, Instant)> =
    once_cell::sync::Lazy::new(|| {
        (std::time::SystemTime::now(), Instant::now())
    });

pub(crate) fn current_time() -> f64 {
    let (system_epoch, instant_epoch) = &*EPOCH;
    // Calculate elapsed time from the monotonic clock (unaffected by NTP)
    let monotonic_elapsed = instant_epoch.elapsed().as_secs_f64();
    // Add to the system epoch to get a Unix-timestamp-compatible value
    // that is still monotonic because it's based on Instant::elapsed()
    system_epoch
