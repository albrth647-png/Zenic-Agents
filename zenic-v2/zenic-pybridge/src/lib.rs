...
//! - `certifier`: Blueprint Certification — template → CertifiedBlueprint, HMAC-SHA256 signing, Phase 5 bridge (Phase 6.D)
...
fn _zenic_native(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // Blueprint Certification (Phase 6.D) — Template → CertifiedBlueprint, HMAC-SHA256 signing
