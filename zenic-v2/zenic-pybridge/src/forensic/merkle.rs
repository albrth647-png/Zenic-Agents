//! Merkle chain verification, proof generation, and batch verification.
...
pub fn verify_merkle_chain(py: Python<'_>, entries: &Bound<'_, PyList>) -> PyResult<Py<PyDict>> {
    ...
    if !parsed.is_empty() {
        root_hash = parsed.last().ok_or_else(|| PyValueError::new_err("no entries parsed"))?.1.clone();
    }
...
pub(crate) fn build_merkle_tree_with_proof(leaves: &[String], target_idx: usize) -> (String, Vec<String>) {
    ...
    while current_level.len() > 1 {
        ...
        if current_level.len() % 2 != 0 {
            current_level.push(current_level.last().expect("invariant: current_level.len() > 1 guarantees last() is Some").clone());
        }
