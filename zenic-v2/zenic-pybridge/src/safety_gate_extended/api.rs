...
pub fn safety_validate_extended(
    ...
    // Step 3: Compliance checks (HARD BLOCK)
    ...
    if let Some(niche_category) = parse_niche_category(category_str) {
        if let Some(standards) = CATEGORY_COMPLIANCE.get(&niche_category) {
            for standard in standards {
                ...
                // Compliance failures for critical violations → DENY (hard block)
    ...
    let reason = if final_verdict == SafetyVerdict::Deny {
        if has_critical_violation {
            "Action denied: critical compliance violation".to_string()
        } else {
            "Action denied: base rule, domain rule, compliance failure, or sensitivity escalation".to_string()
        }
    } else if current_verdict == SafetyVerdict::Approve && compliance_results.iter().any(|r| r.risk_level == "high" && !r.compliant) {
        "Action escalated to APPROVE: high-risk compliance violation".to_string()
