...
use super::types::{ComplianceBlockError, ComplianceResult, ComplianceStandard};
...
impl ComplianceEngine {
    ...
    /// FIX: Enforce compliance — HARD-BLOCK actions that violate critical/high
    /// regulatory requirements. Returns Err with blocking violations if any
    /// result is non-compliant at critical or high severity.
    ///
    /// Previously, compliance checks were advisory-only (returned results but
    /// never blocked actions). Now, critical and high violations escalate to
    /// hard-blocks, preventing the action from proceeding.
    pub fn enforce(
        &self,
        category: NicheCategory,
        action_type: &str,
        config: &serde_json::Value,
    ) -> Result<Vec<ComplianceResult>, ComplianceBlockError> {
        let results = self.check_category(category, action_type, config);
        let blocking: Vec<&ComplianceResult> = results.iter().filter(|r| r.is_blocking()).collect();
        if blocking.is_empty() {
            Ok(results)
        } else {
            Err(ComplianceBlockError {
                blocked_by: blocking.iter().map(|r| r.standard).collect(),
                violations: blocking.iter().flat_map(|r| r.violations.clone()).collect(),
                message: format!(
                    "Action BLOCKED by {} compliance violation(s). Critical/high severity violations must be resolved before proceeding.",
                    blocking.len()
                ),
            })
        }
