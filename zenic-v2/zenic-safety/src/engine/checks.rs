...
mod tests {
    ...
    #[test]
    fn high_risk_compliance_violation_escalates_to_approve() {
        let gate = DomainSafetyGate::new();
        // ISO 27001: infrastructure_change without change_management → high risk.
        // GreenTech has ISO 27001 as a compliance standard.
        // This config intentionally avoids matching any GreenTech domain rules.
        let config = serde_json::json!({"infrastructure_change": "server_migration"});
        let result = gate.check(
            "system_modify",
            &config,
        ...
        // Check that a high-risk compliance violation was found
        let has_high = result.compliance_results.iter().any(|cr| {
            cr.risk_level == "high" && !cr.compliant
        });
        if has_high {
            // Verdict must be escalated to at least APPROVE
            assert!(
                result.final_verdict >= SafetyVerdict::Approve,
                "Expected verdict >= APPROVE due to high-risk compliance violation, got {:?}",
                result.final_verdict
            );
            assert!(
                result.reason.contains("High-risk compliance violation"),
                "Expected reason to mention high-risk compliance violation, got: {}",
                result.reason
            );
        }
    }
    ...
    #[test]
    fn critical_violation_takes_precedence_over_high() {
        let gate = DomainSafetyGate::new();
        // PHI without encryption → critical (HIPAA).
        // This should result in DENY even if there are also high-risk violations.
        let config = serde_json::json!({"data_type": "phi", "action": "access"});
        let result = gate.check(
            "data_access",
            &config,
