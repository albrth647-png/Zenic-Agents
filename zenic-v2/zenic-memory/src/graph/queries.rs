...
impl SemanticGraph {
    ...
    /// Fetches a mapping by its unique mapping_id.
    ///
    /// Returns `Ok(Some(mapping))` if found, `Ok(None)` if not found.
    pub fn get_mapping_by_id(
        &self,
        mapping_id: &str,
    ) -> Result<Option<SemanticMapping>, MemoryError> {
        let mut stmt = self
        ...
        let result = stmt
            .query_row(params![mapping_id], |row| Ok(row_to_mapping(row)))
            .optional()
            .map_err(|e| MemoryError::Database(e.to_string()))?;
...
///
/// Uses proper error propagation instead of `get_unwrap()` to avoid
/// panics on malformed database rows.
fn row_to_mapping(row: &rusqlite::Row<'_>) -> SemanticMapping {
    let mapping_id: String = row.get(0).unwrap_or_default();
    let origin: String = row.get(1).unwrap_or_default();
    let relation: String = row.get(2).unwrap_or_default();
    let destination: String = row.get(3).unwrap_or_default();
    let mechanism_str: String = row.get(4).unwrap_or_default();
    let confidence: i32 = row.get(5).unwrap_or(0);
    let tenant_id: String = row.get(6).unwrap_or_else(|_| "__unknown__".to_string());
    let created_at: i64 = row.get(7).unwrap_or(0);
    let approved_int: i32 = row.get(8).unwrap_or(0);
    let merkle_hash: Option<String> = row.get(9).unwrap_or(None);
