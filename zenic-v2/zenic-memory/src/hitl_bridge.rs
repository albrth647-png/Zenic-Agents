...
impl HitlBridge {
    ...
    ///
    /// FIX: Validates the mandatory fields BEFORE writing them to the request.
    /// Previously, the fields were written first and then validated, which
    /// allowed a caller to pass invalid values that would then be set on the
    /// request before validation — making the GRIETA 3 check meaningless.
    ///
    ...
    pub fn approve(
        ...
        // FIX: Validate mandatory fields BEFORE modifying the request.
        // GRIETA 3 mandatory validation — must pass before any state mutation.
        if !admin_evidence_review {
            return Err(MemoryError::Internal(
                "GRIETA 3 violation: admin_evidence_review must be true".to_string(),
            ));
        }
        if admin_justification.len() < 50 {
            return Err(MemoryError::Internal(format!(
                "GRIETA 3 violation: admin_justification must be >= 50 chars (got {})",
                admin_justification.len()
            )));
        }
        if !risk_acknowledgment {
            return Err(MemoryError::Internal(
                "GRIETA 3 violation: risk_acknowledgment must be true".to_string(),
            ));
        }
        if admin_session_id.is_empty() {
            return Err(MemoryError::Internal(
                "GRIETA 3 violation: admin_session_id must be non-empty".to_string(),
            ));
        }
        if !Self::validate_session(&admin_session_id) {
            return Err(MemoryError::Internal(
                "GRIETA 3 violation: admin_session_id must be a valid hex session ID (>= 32 hex chars)".to_string(),
            ));
        }

        // All validations passed — now it's safe to mutate the request
        ...
        // Double-check with the request's own validate() method
        request.validate()?;
