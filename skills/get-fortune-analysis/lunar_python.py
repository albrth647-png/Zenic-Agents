...
def get_cyber_divination_data(birth_year, birth_month, birth_day, birth_hour=0, birth_minute=0):
    ...
    # 得令 (+40)
    if RELATIONSHIP[dm_element][m_ele] in ["同", "被生"]:
        score += 40
    elif RELATIONSHIP[dm_element][m_ele] == "被克":
        score -= 20

    # 得地 (+15/each)
    for zhi in [ba_zi.getYearZhi(), ba_zi.getDayZhi(), ba_zi.getTimeZhi()]:
        if ZHI_WU_XING[zhi] == dm_element:
            score += 15
    ...
    return {
        ...
        "bazi": {"day_master": day_master, "element": dm_element, "strength": strength_cn, "score": score},
        "fortune": {
